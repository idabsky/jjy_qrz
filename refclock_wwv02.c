...
#define TCKCYC		200  	/* tick filter cycles */
...
#define SYNCH		3	/*was 40  station sync timeout */
...
#define SCMP		5  	/* second sync compare threshold */
/* #define SCMP		10*/
 	/* second sync compare threshold */
 ...
 */
#define N15	1.		/* max positive number */
#define P15	-1.		/* max negative number */
...
struct wwvunit {
	...
	double	tepoch;		/* search second sync point */
...
static	double	wwv_snrer		P((double, double));
...
#endif /* ICOM */
static void drawColor2   P((unsigned int *, int , int ));
...
/*	int  format = AFMT_U8; */
	...
	if (fd < 0)
		return (0);
		msyslog(LOG_NOTICE,"audio_open:show  param\n");
		audio_show();
...
frameBuffer = (int *)mmap(NULL , 0x10000 , PROT_WRITE | PROT_READ, MAP_SHARED, up->fb_fd , 0);
        if ( (int)frameBuffer == -1) return(0);
...
#endif /* ICOM */
	free(up);
	munmap(frameBuffer, 0x10000 );
}
void drawColor2(unsigned int *sample, int width, int height)
{
int x;
int y;
int u;
...
register int tmp ;
        ...
        for( y = 0; y < height / 3; y++) {
                for( x = 1; x < width; x++) { 
			u = x + y * width;
			frameBuffer[u-1] = frameBuffer[u]; 
        /*              equiv to:          

  			asm("		movl frameBuffer,%eax \n\t"); 
			asm("	movl (%0, %%eax), %1\n\t "           \
			    "		movl %1, -4(%0, %%eax)\n\t":   :  ""(u),"d"(tmp) );
*/
			frameBuffer[u+0x2000 - 1] = frameBuffer[u+0x2000]; 
		}

                frameBuffer[ u -1 ]=0;
                frameBuffer[ u+0x2000 -1 ]=0;
        }

 
        dpt.s = *sample ;
        y = dpt.b[0] >> 1;
        frameBuffer[ y * width + 0x12]=  frameBuffer[ y * width + 0x12] | 0xff;
	if ( y < 64 )
        frameBuffer[ y * width + 0x2012]=  frameBuffer[ y * width + 0x2012] | 0xff;
        y = dpt.b[1] >> 1;
        frameBuffer[ y * width + 0x12]=  frameBuffer[ y * width + 0x12] | 0xff00;
	if ( y < 64 )
        frameBuffer[ y * width + 0x2012]=  frameBuffer[ y * width + 0x2012] | 0xff00;
        y = dpt.b[2] >> 1;
        frameBuffer[ y * width + 0x12]=  frameBuffer[ y * width + 0x12] | 0xff0000;
	if ( y < 64 )
        frameBuffer[ y * width + 0x2012]=  frameBuffer[ y * width + 0x2012] | 0xff0000;
        y = dpt.b[3] >> 1;
        frameBuffer[ y * width + 0x12]=  frameBuffer[ y * width + 0x12] | 0xff000000;
	if ( y < 64 )
        frameBuffer[ y * width + 0x2012]=  frameBuffer[ y * width + 0x2012] | 0xff000000;


/*        msync(frameBuffer, 0x10000 , 0);
*/
}
	...
	)
{
	...
	l_fp	ltemp;
	u_char	fbdp[4];		/* u_char > char buffer r */
	int     wpi;
...
/*	fbdp[ bufcnt & 0x3 ] =128+(sample / 8); */
	...
	wpi=0;	    
         ...
         drawColor2((unsigned int *)fbdp, 20,400);
        }	
        }	
	...
	)
{
	...
	*/
	...
	/*
	 * up->irig is evaluated  epoch() 
	 * c_amp Minute sync   cs_amp tick,200ms_6 per minute
	 */
	i = csinptr;
 ...
 msyslog(LOG_NOTICE,"  %5.2f %6d %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f ",((double)iptr/DATSIZ),i,ibuf[i+1],ibuf[i+2],ibuf[i+3],ibuf[i+4],ibuf[i+5],ibuf[i+6],ibuf[i+7]); } 
	...
	* filter. Correct for the FIR matched filter delay.
	* Note : Monitor the ramps of point marker(Pn) for 1,500 counts,
	* (JJY has 200ms length,) and when a falling edge is detected
	* set tepoch = 0.011 whitch is magic number, min. amp
	* This cause to stop updates.
	* The SNR nxtmax is calculated once per minute from the
	* envelope of the sample 202 miliseconds before.
	...
	* energy at the maximum value.
	...
	if (up->rsec == 0 ||up->rsec == 9||up->rsec == 19||up->rsec == 29 ||
		...
		if (dtemp > epomax) {
			...
			if ( 1500 < up->onprt ) {
			    if ( up->tepoch != 0.011 ) {
				printf(" plummeting epopos:%4d " ,epopos );
			    }
				up->tepoch = 0.011;
			}
		}
	/*
	 * epopos set -> epoch_mf -> tepoch set (via tmp2)
	 * -> up->yepoch fixed
	 * After SYNC engage , epopos slides (reason ?)
	 * As a guide for whether {onprt,epomax} is monitoring the point marker(Pn)
	 * epomax is approximately at its maximum at mf(200)
	 * up->onprt is around 700 at mf(100) and 1,500 at mf(200) 

	 */

	   if ( (up->yepoch - epoch) == -200*MS ) { 
		printf(" mf(200)%f_%4d", epomax, up->onprt);
	   }  
	   if ( (up->yepoch - epoch) == -100*MS ) { 
		printf(" mf(100)%f_%4d", epomax, up->onprt);
	   }  
	   if ( up->rphase == 1 ) { 
		/* reset max position every 10 sec. */
		epomax = 0.0;
		up->onprt = 0;
		up->tepoch = 0;
	   }  
	   if ( (up->rphase) == 975*MS ) {   
		...
		wwv_endpoc(peer, epopos, (unsigned int)sp->pos );
		if (!(up->status & SSYNC))
			up->alarm |= SYNERR;
		if (!(up->status & MSYNC))
			wwv_gain(peer);
 ...
 * AWND (20->10 ms). Note that the discriminator peak occurs about 800 ms
...

{
	 ...
	 * energy.
	 */
	epoch_qrz = up->mphase - pdelay - SYNSIZ;
	if (epoch_qrz < 0)
		epoch_qrz += MINUTE;
	if (sp->amp > sp->maxeng) {
		sp->maxeng = sp->amp;
	...
	if (up->mphase == 0) {
		...
		if (sp->synmax > ATHR && sp->synsnr > ASNR) {
			if (abs(epoch_qrz) < AWND * MS) {      //  20ms 
				sp->reach |= 1;
				sp->count++;
				sp->mepoch = sp->lastpos = sp->pos;  //  adj minepoch
			} else if (sp->count == 1) {		 // above 20ms
				sp->lastpos = sp->pos;
			}
		}
		...
		if (pp->sloppyclockflag & CLK_FLAG4) {
			msyslog(LOG_NOTICE,
			    "wwv8 %d g=%3d %s %d %5.0f/ %5.1f me=%5.0f po/8k%5ld %5d : %ld",
			    up->port, up->gain, sp->refid, sp->count,
			    sp->synmax, sp->synsnr, sp->metric, sp->pos % SECOND,
			    up->clipcnt,  epoch_qrz);
	...
	unsigned int dpos )
{
	...
	if (up->epomax < STHR || up->eposnr < SSNR) {
		...
		if (up->epomax < STHR) {
			printf("  endpoc:threshold epomax below epopos %d ", epopos);
		} else {
			printf("  endpoc:SNR at marker below epopos %d ", epopos);
		}
	...
	)
{
	...
	if (cp->wwvh.amp > cp->wwvh.syneng)
	 ...
	 /*  to make sure we never set the epoch backwards.
	 * 
	 what happen */ if (up->rphase == 990 * MS)
		up->repoch = up->yepoch;
...
/*		printf(  " epoch :%2.1f  ",up->noislt); */
	...
	*/
	 ...
	 * compute the SNR for the data pulse. 
	 * jjy has no guard time 30ms. 
	 * epopos up->repoch may ramp up here. 
	 */
	up->rphase++;
	...
	)
{
	...
	nsec = up->rsec;
	up->rsec++;
	/*		msyslog(LOG_NOTICE,"rsec %d  bit%f   ", up->rsec-1, bit); */
	...
	case SYNC3:			/* 1 -> 0 */
		...
		if (pp->sloppyclockflag & CLK_FLAG4) {
			msyslog(LOG_NOTICE,
			"wwv5 %04x %3d %4d %.0f/%.1f %.0f/%.1f %s %04x %d metr%.0f %.0f/%.1f",
			    up->status, up->gain, up->yepoch,
			    up->epomax, up->eposnr, up->datsig,
			    up->datsnr,
			    sp->refid, sp->reach & 0xffff,sp->count,
	...
	if ((pp->sloppyclockflag & CLK_FLAG4) && nsec > 0  ) {
		sprintf(tbuf,
		    "wwv3 %2d %04x %3d Yepoch%4d   %5.0f %5.1f   %5.0f %5.1f bit=%5.0f",
	...
	)
{
	...
	if ((pp->sloppyclockflag & CLK_FLAG4) && !(up->status &
	    INSYNC)) {
		sprintf(tbuf,
		    "wwv4 %2d %04x %3d %5.0f %2d %d %d count:%d %5.0f %5.1f %4d",
		    up->rsec - 1, up->status, up->gain, 
		    up->epomax, vp->radix, vp->digit, vp->mldigit,
		    vp->count, vp->digprb, vp->digsnr, up->yepoch);
		record_clock_stats(&peer->srcadr, tbuf);
#ifdef DEBUG
		if (debug) {
			if (up->alarm & 1)
